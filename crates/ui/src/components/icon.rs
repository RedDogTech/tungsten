...
use crate::{prelude::*, rems_from_px, Color, Indicator};
...
impl RenderOnce for DecoratedIcon {
    fn render(self, cx: &mut WindowContext) -> impl IntoElement {
        let background = self
            .parent_background
            .unwrap_or(cx.theme().colors().background.into());
...
impl RenderOnce for IconWithIndicator {
    fn render(self, cx: &mut WindowContext) -> impl IntoElement {
        let indicator_border_color = self
            ...
            .unwrap_or_else(|| cx.theme().colors().elevated_surface_background.into());
